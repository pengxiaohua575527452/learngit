<template>
    <div class='title-component' :style="{paddingLeft: 20*ratio + 'px',paddingRight: 20*ratio + 'px',paddingTop: 14*ratio + 'px',paddingBottom: 41*ratio + 'px'}">
        ...
        <h4 :style="{marginTop: 10*ratio + 'px',lineHeight: 25*ratio + 'px',fontSize: 18*ratio + 'px'}">
          {{newsDetailData.subhead}}
        </h4>
        <p :style="{marginTop: 10*ratio + 'px',lineHeight: 20*ratio + 'px',fontSize: 14*ratio + 'px'}">
          {{newsDetailData.publishTime | formatDate }}
        </p>
        <div class='btn-box' :style="{marginTop: 10*ratio + 'px',height: 40*ratio + 'px'}">
            <a :href= newsDetailData.attachments class='btn-down' :style="{height: 40*ratio + 'px',width: 40*ratio + 'px'}">
        ...
        <div class='content-box' ref='contentBox' v-html="newsDetailData.htmlContent">
          {{newsDetailData.htmlContent}}
        </div>
        <!-- 返回首屏 -->
        <div
          class='btn-top'
...
export default {
  data: function() {
    return {
      ...
      // newsDetailData: this.newsDetailData
      newsDetailData: []
    };
  },
  methods:{
    ...
    shareFn:function(){
      ...
      dd.biz.util.share({
          ...
          title: vm.newsDetailData.title, //分享页面的标题
          content: vm.newsDetailData.subhead,//分享页面的内容
  ...
  mounted: function() {
    ...
    this.$nextTick(function() {
        ...
        var languageType = this.$route.query.language
        ...
        // alert('进入详情页')
         $.ajax({
          type: 'Get',
          // type: 'POST',
          // url: root + "/ir/news/newsdetail",
          // url:"http://test.mmany.cn/investment/ir/news/newsdetail",
          url:root + '/investment/ir/news/newsdetail',
          data: {
          ...
          success: function (data) {
            // alert('请求成功了')
            // alert('详情页数据： ' +JSON.stringify(data))
            vm.newsDetailData = data;
          },
          error:function(data){
            // alert('请求失败了')
            // alert('error : '+ data)
